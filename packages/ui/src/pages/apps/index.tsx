...
import { List, Input, Dropdown, Space, Avatar, Tree } from 'antd';
...
const accessAppData = (appData: any):AppDataType[] => {
  return Object.keys(appData).map((path) => {
    ...
    // 添加树形控件中展示路由的数据
    let target = result.treeData[0].children.find((val: DataNode) => val.title === '路由信息');
...
const Header:React.FC<HeaderProps> = ({ onSearch }) => {
  ...
  return (
    <Space>
      <Dropdown.Button menu={{ items, onClick: handleMenuClick }}>更多操作</Dropdown.Button>
...
function AppsPage() {
  ...
  const handleSearch = (value: string) => {
    setFilterDatas(appDatas.filter((item: AppDataType) => item.name.includes(value)))
  }
