 ...
 * @param dryRun 确认存在
 */
export function makeSureMaterialsTempPathExist(dryRun, nodeModulesPath) {
  let userHome = nodeModulesPath;
  if (!nodeModulesPath) {
    userHome = process.env.NODE_ENV === 'test' ? '/Users/test' : homedir();
  }
 ...
 * @param args
 */
export function downloadFromGit(
  url,
  id,
  branch = 'master',
  log,
  ...
  const { dryRun, nodeModulesPath } = args;
...
const gitSiteParser =
  // eslint-disable-next-line no-useless-escape
  /^(https\:\/\/|http\:\/\/|git\@)((github|gitlab)[\.\w\-]+|(?:(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))(\/|\:)([\w\-]+)\/([\w\-]+)(\/tree\/([\w\.\-]+)([\w\-\/]+))?(.git)?$/;
